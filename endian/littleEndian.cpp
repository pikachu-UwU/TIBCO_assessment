...
unsigned short my_ntohs(unsigned short in) {
    ...
    last_byte = (last_byte << 2) & 0xFF00;
    first_byte = (first_byte >> 2) & 0x00FF;
...
unsigned int my_ntohl(unsigned int in) {
    ...
    // STEP2 - shift the bytes into correct positions:
    forth_byte = (forth_byte << 6) & 0xFF000000;
    third_byte = (third_byte << 6) & 0x00FF0000;
    second_byte = (second_byte << 6) & 0x0000FF00;
    first_byte = (first_byte << 6) & 0x000000FF;
